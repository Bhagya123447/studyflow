  ...
  return (
    <div className="stats-page-container">
      ...
      <div className="stats-grid">
        ...
        <div className="stats-card">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={chartData}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="var(--chart-grid-color, #f0f0f0)" />
              <XAxis dataKey="day" axisLine={false} tickLine={false} style={{ fontSize: '12px' }} />
              <YAxis axisLine={false} tickLine={false} style={{ fontSize: '12px' }} />
              <Tooltip cursor={{ fill: 'rgba(0,0,0,0.05)' }} />
              <Bar dataKey="focusTime" fill="var(--primary-color-light, #6366F1)" name="Minutes Focused" radius={[10, 10, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
          ...
          <ResponsiveContainer width="100%" height={250}>
            <LineChart data={chartData}>
              ...
              <Line
                type="monotone"
                dataKey="tasksDone"
                stroke="var(--accent-color-green, #10B981)"
        ...
        {/* Subject vs Time */}
        <div className="stats-card full-width"> {/* Use full-width class for span */}
          <h2 className="stats-card-title">Time Spent by Subject</h2>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={subjectChart}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="var(--chart-grid-color, #f0f0f0)" />
              <XAxis dataKey="subject" axisLine={false} tickLine={false} style={{ fontSize: '12px' }} />
              <YAxis axisLine={false} tickLine={false} style={{ fontSize: '12px' }} />
              <Tooltip cursor={{ fill: 'rgba(0,0,0,0.05)' }} />
              <Bar dataKey="minutes" fill="var(--accent-color-orange, #F59E0B)" name="Minutes" radius={[10, 10, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
